<?php
   /*友情链列表接显示类 基于PHP7 Design By 小远*/
  class ListFrinedLinks
  {
	  public $Data;
	  public $totalPages;
	  public $page;
	  ...
	  function _construct($Data)
	  {
		  ...
		  $sql = "select ID  from friendlinks";
		  ...
		  $sql = "select * from friendlinks  limit $startNum,$maxNum";
	 ...
	 public function listItems()
	  {
		  if($this->resultSet)
		  {
			  do
			  {
				  $name = $this->row['LinkName'];
				  $url = $this->row['LinkUrl'];
				  if($this->page == 1)
			     {
				  echo "<li class='list'><a href='$url' title='$name' target='_blank'>$name</a><img class='listhot' src='images/hot.gif' alt='图片关键词' /></li>";
			     }
			     else
			     {
					  	 echo "<li class='list'><a href='$url' target='_blank'>$name</a></li>";
			  }
